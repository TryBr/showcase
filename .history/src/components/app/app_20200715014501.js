...
export default class App extends Component {
  ...
  onFilterChange = (type, filter) => {
    let filterItemIndex = this.state.filters.findIndex(item => item[type]);
    if (filterItemIndex) {
      this.setState(previousState => ({
        filters: [...previousState.filters[filterItemIndex], {[type]: filter}]
      }));
    } else {
      this.setState(previousState => ({
        filters: [...previousState.filters, {[type]: filter}]
      }));
    }
